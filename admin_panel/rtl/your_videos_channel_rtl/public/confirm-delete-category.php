<?php 
	...
	// get image file from table
	$sql_query = "SELECT video_thumbnail FROM tbl_gallery WHERE cat_id = ?";
	...
	if ($stmt_menu->prepare($sql_query)) {	
		...
		$stmt_menu->bind_result($image_data['video_thumbnail']);
	}
			
	// delete all menu image files from directory
	while ($stmt_menu->fetch()) {
		$video_thumbnail = $image_data[video_thumbnail];
		$delete_image = unlink('upload/'."$video_thumbnail");
	}
